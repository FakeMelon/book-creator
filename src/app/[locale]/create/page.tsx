    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-white to-rose-50">
        <div className="text-center space-y-6">
          ...
          <h1 className="font-display text-3xl font-bold">{t("generatingTitle")}</h1>
          <p className="text-muted-foreground">{demo.genProgress}</p>
          <p className="text-sm text-muted-foreground">{t("generatingTime")}</p>
      ...
      <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-rose-50">
        ...
        <main className="max-w-2xl mx-auto px-4 py-8">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground mt-2">{t("storyFor", { childName: store.childName })}</p>
              ...
              <div className="mt-4 max-w-sm mx-auto space-y-2">
                <p className="text-sm text-muted-foreground">
                  {demo.imageProgress.stage === "character-ref"
                    ? t("generatingCharRef")
                    : t("generatingIllustrations", { done: demo.imageProgress.done, total: demo.imageProgress.total })}
                </p>
                <Progress
            ...
            {demo.imageProgress?.stage === "done" && (
              <p className="text-sm text-green-600 mt-3">{t("allIllustrationsGenerated")}</p>
            )}

            {!demo.imageProgress && (
              <p className="text-xs text-muted-foreground mt-1">
                {store.photoKey
                  ? t("preparingIllustrations")
                  : t("textOnlyNoPhoto")}
              </p>
            )}
          </div>
