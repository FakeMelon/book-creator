...
export function StepCreativeDirection() {
  ...
  // Generic card preset renderer used by theme, subject, and storyHeart sentences
  const renderCardPresets = useCallback(
    (
      items: { id: string; emoji?: string; icon?: string }[],
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {items.map((item) => (
            ...
            >
              ...
              <span className="text-sm font-semibold">{labelFn(item.id)}</span>
              {options?.descriptionFn && (
  ...
  const customRenderers: Record<string, ReturnType<typeof renderCardPresets> | undefined> = useMemo(() => ({
    theme: renderCardPresets(
      THEMES.map((t) => ({ id: t.id, emoji: t.icon })),
      (id) => tConst(`themes.${id}.name`),
      { descriptionFn: (id) => tConst(`themes.${id}.description`) },
    ),
